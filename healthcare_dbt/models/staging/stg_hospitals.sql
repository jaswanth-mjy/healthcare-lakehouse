...
cleaned AS (
    SELECT
        ...
        -- ── Overall Rating ───────────────────────────────────────────────────
        CASE
            WHEN regexp_matches(TRIM(hospital_overall_rating), '^[1-5]$')
            THEN TRY_CAST(hospital_overall_rating AS INTEGER)
            ELSE NULL
        END                                           AS overall_rating,

        -- ── Mortality (MORT) Measures ─────────────────────────────────────────
        TRY_CAST(count_of_facility_mort_measures AS INTEGER)  AS mort_measures_count,
        ...
        -- ── Safety Measures ───────────────────────────────────────────────────
        TRY_CAST(count_of_facility_safety_measures AS INTEGER) AS safety_measures_count,
        ...
        TRY_CAST(count_of_facility_readm_measures AS INTEGER) AS readm_measures_count,
        ...
        TRY_CAST(count_of_facility_pt_exp_measures AS INTEGER) AS pt_exp_measures_count,

        -- ── Timeliness & Efficiency (TE) ──────────────────────────────────────
        TRY_CAST(count_of_facility_te_measures AS INTEGER)    AS te_measures_count,

        -- ── Derived Quality Score (0–100) ─────────────────────────────────────
        -- Simple score: % of measures rated "Better" across MORT + Safety + READM
        CASE
            WHEN (
                COALESCE(TRY_CAST(count_of_facility_mort_measures AS INTEGER), 0) +
                COALESCE(TRY_CAST(count_of_facility_safety_measures AS INTEGER), 0) +
                COALESCE(TRY_CAST(count_of_facility_readm_measures AS INTEGER), 0)
            ) = 0 THEN NULL
            ELSE ROUND(
                100.0 * (
                    COALESCE(TRY_CAST(count_of_mort_measures_better AS INTEGER), 0) +
                    COALESCE(TRY_CAST(count_of_safety_measures_better AS INTEGER), 0) +
                    COALESCE(TRY_CAST(count_of_readm_measures_better AS INTEGER), 0)
                ) / (
                    COALESCE(TRY_CAST(count_of_facility_mort_measures AS INTEGER), 0) +
                    COALESCE(TRY_CAST(count_of_facility_safety_measures AS INTEGER), 0) +
                    COALESCE(TRY_CAST(count_of_facility_readm_measures AS INTEGER), 0)
